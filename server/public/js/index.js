...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _timer = __webpack_require__(302);

	var _timer2 = _interopRequireDefault(_timer);

	var _calculate = __webpack_require__(303);
	...
	function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } } // 模块的整合
	  ...
	  try {
	    for (var _iterator = Reflect.ownKeys(source)[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
	      ...
	      if (key !== 'constructor' && key !== 'prototype' && key !== 'name') {
	        //过滤
	        var desc = Object.getOwnPropertyDescriptor(source, key); // 获取指定对象的自身属性描述符
	...
	var Lottery = function (_mix) {
	  ...
	  function Lottery() {
	    ...
	    _this.number = new Set(); //奖号
	    _this.issue_el = '#curr_issue';
	    _this.countdown_el = '#countdown'; //倒计时的选择器
	    _this.state_el = '.state_el'; //状态的选择器
	    _this.cart_el = '.codelist'; //购物车的选择器
	    _this.omit_el = ''; //遗漏
	    _this.cur_play = 'r5'; //当前的默认玩法
	    _this.initPlayList();
	    _this.initNumber();
	  ...
	  _createClass(Lottery, [{
	    ...
	    value: function updateState() {
	      ...
	      this.getState().then(function (res) {
	        ...
	        (0, _jquery2.default)(self.issue_el).text(res.issue); //更新期号
	...
	var Base = function () {
	  ...
	  _createClass(Base, [{
	    ...
	    value: function changePlayNav(e) {
	      ...
	      (0, _jquery2.default)('.boll-list .btn-boll').removeClass('btn-boll-active'); //清空上次的选择
	      self.getCount();
	    }
	    ...
	    value: function addCode() {
	      ...
	      var active = $active ? $active.length : 0; //注意这里
	    ...
	    value: function getRandom(num) {
	      var arr = [],
	          index = void 0;
	      var number = Array.from(this.number); //1~11号码数组
	      while (num--) {
	...
	var Calculate = function () {
	  ...
	  _createClass(Calculate, [{
	    ...
	    value: function computeCount(active, play_name) {
	      var count = 0;
	      var exist = this.play_list.has(play_name); //判断玩法列表里面是否有这样的玩法,set形式
	      var arr = new Array(active).fill('0'); //生成长度为active的数组，并填充为0
	    ...
	    value: function computeBonus(active, play_name) {
	      var play = play_name.split(''); //得到当前玩法,例如R2
	      var self = this;
	      var arr = new Array(play[1] * 1).fill(0);
	      var min = void 0,
	          max = void 0;
	      if (play[0] === 'r') {
	        var min_active = 5 - (11 - active); //最小命中数
	        if (min_active > 0) {
